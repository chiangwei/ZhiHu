...
class RegisteationForm(FlaskForm):
    ...
    password = PasswordField('密码',validators=[Required(),EqualTo('password2', message=u'两次输入的密码必须一致')],render_kw={'aria-label':u'密码', 'placeholder': u'密码','type':'password','autocomplete':'off'})
    password2 = PasswordField('确认密码',validators=[Required()],render_kw={'aria-label':u'确认密码', 'placeholder': u'确认密码','type':'password','autocomplete':'off'})
...
class ChangePasswordForm(FlaskForm):
    old_password = PasswordField(u'旧密码',validators=[Required()],render_kw={'aria-label':u'旧密码', 'placeholder': u'旧密码'})
    password = PasswordField(u'新密码',validators=[Required(),EqualTo('password2',message=u'两次输入的密码必须一致')],render_kw={'aria-label':u'新密码', 'placeholder': u'新密码'})
...
class PasswordResetForm(FlaskForm):
    email = StringField(u'邮箱地址',validators=[Required(),Length(1,64),Email()])
    password = PasswordField(u'新的密码', validators=[Required(),EqualTo('password2',message=u'两次输入的密码必须一致')])
    password2 = PasswordField(u'确认新密码',validators=[Required()])
